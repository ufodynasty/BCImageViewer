...
/* Checks logging format against a whitlist of format specifiers. */
int check_log_msg(const char *logging_fmt) {
    int s = 1, ld = 2;
    for(int i = 0; logging_fmt[i]; i++) {
        if(logging_fmt[i] == '%') {
            i++;
            if(logging_fmt[i] == '%') {/* Unlimited printable % characters are allowed. */
                continue;
            }
            if(logging_fmt[i] == 's' && s != 0) {/* One or fewer string formats are allowed. */
                s--;
            } else if(!memcmp(&logging_fmt[i],"ld",2) && ld != 0) {/* Two or fewer decimal formats are allowed */
                ld--;
                i++;
            } else {
                fprintf(stderr, "Invalid logging string\n");
                return 1;
            }
        }
    }
...
void print_log_msg(struct image_info *info) {
    ...
    if(check_log_msg(logging_fmt)) {
        printf("Displaying image of width %ld and height %ld"
    " from %s", info->width, info->height, time_str,
           info->create_time);
    } else {
